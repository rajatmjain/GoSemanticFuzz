...
func (int64Schema Int64Schema) CustomInt64FuzzFunc() func(i64 *int64,c Continue){
	int64Schema.checkInt64()
	return func (i64 *int64,c Continue){
		*i64 = int64Schema.randInt64(c.Rand)
	}

}

func (int32Schema Int32Schema) CustomInt32FuzzFunc() func(i32 *int32,c Continue){
	int32Schema.checkInt32()
	return func (i32 *int32,c Continue){
		*i32 = int32Schema.randInt32(c.Rand)
	}

}

func (float64Schema Float64Schema) CustomFloat64FuzzFunc() func(f64 *float64,c Continue){
	float64Schema.checkFloat64()
	return func (f64 *float64,c Continue){
		*f64 = float64Schema.randFloat64(c.Rand)
	}

}

func (float32Schema Float32Schema) CustomFloat32FuzzFunc() func(f32 *float32,c Continue){
	float32Schema.checkFloat32()
	return func (f32 *float32,c Continue){
		*f32 = float32Schema.randFloat32(c.Rand)
	}
...
func (ur UnicodeRange) check() {
	if (ur.Last < ur.First) || ur.MaxLength==0{
		panic("The last encoding must be greater than the first one and maximum length can not be 0.")
	}
}

func (int64Schema Int64Schema) checkInt64(){
	if(int64Schema.Minimum>=int64Schema.Maximum){
		panic("The Maximum must be greater than the Minimum.")
	}
}

func (int32Schema Int32Schema) checkInt32(){
	if(int32Schema.Minimum>=int32Schema.Maximum){
		panic("The Maximum must be greater than the Minimum.")
	}
}

func (float64Schema Float64Schema) checkFloat64(){
	if(float64Schema.Minimum>=float64Schema.Maximum){
		panic("The Maximum must be greater than the Minimum.")
	}
}

func (float32Schema Float32Schema) checkFloat32(){
	if(float32Schema.Minimum>=float32Schema.Maximum){
		panic("The Maximum must be greater than the Minimum.")
	}
}
...
func (ur UnicodeRange) randString(r *rand.Rand) string {
	n := r.Intn(ur.MaxLength-ur.MinLength+1)+ur.MinLength
	sb := strings.Builder{}
	sb.Grow(n)
	for i := 0; i < n; i++ {
...
func (int64Schema Int64Schema) randInt64(r *rand.Rand) int64 {
	rand.Seed(time.Now().UnixNano())
    min := int64Schema.Minimum
    max := int64Schema.Maximum
	difference := max-min
    result := (rand.Int63n(difference) + min)
	
    return result
}

func (int32Schema Int32Schema) randInt32(r *rand.Rand) int32 {
	rand.Seed(time.Now().UnixNano())
    min := int32Schema.Minimum
    max := int32Schema.Maximum
	difference := max-min
    result := (rand.Int31n(difference) + min)
	
    return result
}

func (float64Schema Float64Schema) randFloat64(r *rand.Rand) float64 {
	rand.Seed(time.Now().UnixNano())
	min := float64Schema.Minimum
    max := float64Schema.Maximum
	precision := float64Schema.Precision
	ratio := math.Pow(10, float64(precision))
	result := min + rand.Float64() * (max - min)
	return math.Round(result*ratio)/ratio
}

func (float32Schema Float32Schema) randFloat32(r *rand.Rand) float32 {
	rand.Seed(time.Now().UnixNano())
	min := float32Schema.Minimum
    max := float32Schema.Maximum
	precision := float32Schema.Precision
	ratio := math.Pow(10, float64(precision))
...
var defaultUnicodeRanges = UnicodeRanges{
	{' ', '~',0,20},           // ASCII characters
	{'\u00a0', '\u02af',0,20}, // Multi-byte encoded characters
	{'\u4e00', '\u9fff',0,20}, // Common CJK (even longer encodings)
}
